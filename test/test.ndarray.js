...
var glastIndexOfTruthy = require( './../lib/ndarray.js' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof glastIndexOfTruthy, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( glastIndexOfTruthy.length, 4, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns the index of the last truthy element', function test( t ) {
	...
	// Nonnegative stride...
	actual = glastIndexOfTruthy( x.length, x, 1, 0 );
	t.strictEqual( actual, 4, 'returns expected value' );

	actual = glastIndexOfTruthy( x.length-1, x, 1, 1 );
	t.strictEqual( actual, 3, 'returns expected value' );

	actual = glastIndexOfTruthy( x.length-2, x, 1, 2 );
	t.strictEqual( actual, 2, 'returns expected value' );

	actual = glastIndexOfTruthy( 1, x, 1, 0 );
	t.strictEqual( actual, -1, 'returns expected value' );

	// Negative stride...
	actual = glastIndexOfTruthy( x.length, x, -1, x.length-1 );
	t.strictEqual( actual, 4, 'returns expected value' );

	actual = glastIndexOfTruthy( 3, x, -2, x.length-1 );
	t.strictEqual( actual, 2, 'returns expected value' );

	actual = glastIndexOfTruthy( 3, x, -2, x.length-2 );
	t.strictEqual( actual, 0, 'returns expected value' );

	t.end();
});

tape( 'the function returns the index of the last truthy element (accessors)', function test( t ) {
	var actual;
	var x;
...
tape( 'the function supports an offset parameter', function test( t ) {
	...
	actual = glastIndexOfTruthy( 3, x, 1, 3 );
	t.strictEqual( actual, 2, 'returns expected value' );
...
tape( 'the function ignores falsy elements (e.g., `0`, `NaN`) (accessors)', function test( t ) {
	...
	x = toAccessorArray( [ 0.0, 5.0, NaN, 0.0 ] );
...
tape( 'the function returns `-1` if provided an `N` parameter which is less than or equal to zero', function test( t ) {
	...
	actual = glastIndexOfTruthy( 0, [ 1.0, 2.0, 3.0 ], 1, 0 );
	t.strictEqual( actual, -1, 'returns expected value' );

	actual = glastIndexOfTruthy( -1, [ 1.0, 2.0, 3.0 ], 1, 0 );
	t.strictEqual( actual, -1, 'returns expected value' );
...
tape( 'the function returns `-1` if provided an `N` parameter which is less than or equal to zero (accessors)', function test( t ) {
	...
	actual = glastIndexOfTruthy( 0, toAccessorArray( [ 1.0, 2.0, 3.0 ] ), 1, 0 ); // eslint-disable-line max-len
	t.strictEqual( actual, -1, 'returns expected value' );

	actual = glastIndexOfTruthy( -1, toAccessorArray( [ 1.0, 2.0, 3.0 ] ), 1, 0 ); // eslint-disable-line max-len
	t.strictEqual( actual, -1, 'returns expected value' );
